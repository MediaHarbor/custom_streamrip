...
def rip(ctx, config_path, folder, no_db, quality, codec, no_progress, verbose):
    """custom_streamrip: the all in one music downloader."""
    global logger
    logging.basicConfig(
        level="INFO",
        format="%(message)s",
    ...
    with ctx.obj["config"] as cfg:
        ...
        if version_coro is not None:
            ...
            if latest_version != __version__:
                console.print(
                    f"\n[green]A new version of custom_streamrip [cyan]v{latest_version}[/cyan]"
                    " is available! Run [white][bold]pip3 install custom_streamrip --upgrade[/bold][/white]"
                    " to update.[/green]\n"
                )
